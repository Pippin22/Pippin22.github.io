...
<body>
  ...
  <script>
    ...
    function setPin(newPin) { localStorage.setItem("parentPin", newPin); }
    ...
    function renderFolders() {
      ...
      for (var folderName in folders) {
        ...
        icon.className = "folderIcon"; icon.textContent = "📁";
        card.appendChild(icon);

        var nameDiv = document.createElement("div");
        nameDiv.className = "folderName"; nameDiv.textContent = folderName;
    ...
    function showFolderContents(folderName) {
      ...
      videos.filter(v => v.folder === folderName).forEach(v => {
        ...
        if (v.locked) {
          ...
          overlay.className = "lockOverlay"; overlay.textContent = "🔒 Locked";
